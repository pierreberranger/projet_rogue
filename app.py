...
game = Game(is_multiplayer=False)
...
def on_join_msg(json):
    ...
    if not(player_id in game.getPlayers().keys()):  # new player
        ...
        socketio.emit("you_joined", {"lifes": game.getPlayers()[
                      player_id].getLife()}, to=player_index[player_id])
...
def on_is_hit_msg(json):
    ...
    is_near, is_hit, n_hits, is_dead, monsters_locations, data = game.is_hit(
        player_id)
    if is_near:
...
# we use leave instead of disconnect because it is already used by default: with no arguments given
@socketio.on("leave")
...
# just in case the socket id changes (sometimes due to bad connection)
